...
def homotopy_helpers(num_s, num_p, nums_a, u, phi, eta, nu):
    ...
    T_J_qre_3 = np.einsum('sp...t,tpSP->sp...SP', phi, T_J_qre_temp)
    
    T_J_qre_5 = np.einsum('sptq,tqSP->spSP', T_H_qre_3, T_J_qre_temp)
    
    T_J_qre_2 = np.zeros(shape=(num_s,num_p,*[num_a_max]*(num_p-1),num_s,num_p,num_a_max), dtype=np.float64)
    for s in range(num_s):
        for p in range(num_p):
...
###############################
## logit Markov QRE homotopy ##
###############################
    

def H_qre_nontransformed(y, u, phi, num_s, num_p, nums_a, T_y2beta, homotopy_helpers):
    
    einsum_eqs = homotopy_helpers['einsum_eqs_qre']
    
    ## extract log-strategies beta, state values V and homotopy parameter gamma from y
    beta_withNaN = np.einsum('spaN,N->spa', T_y2beta, y[:nums_a.sum()])
    V = y[nums_a.sum():-1].reshape((num_s,num_p))
    gamma = y[-1]
    ...
    u_tilde = u + np.einsum('sp...S,Sp->sp...', phi, V)
    ...
    H = np.nan * np.ones(nums_a.sum()+num_s*num_p)
    k = 0
    for s in range(num_s):
        for p in range(num_p):
            for a in range(nums_a[s,p]):
                H[k] = - sigma[s,p,a] + np.exp(gamma*Eu_tilde_a[s,p,a]) / np.exp(gamma*Eu_tilde_a[s,p,:]).sum()
                k += 1
    for s in range(num_s):
        for p in range(num_p):
            H[k] = - V[s,p] + Eu_tilde[s,p]
            k += 1
...
def H_qre(y, u, phi, num_s, num_p, num_a_tot, T_y2beta, homotopy_helpers):
    
    H_mask = homotopy_helpers['H_mask']
    T_H = homotopy_helpers['T_H_qre']
    einsum_eqs = homotopy_helpers['einsum_eqs_qre']
    ...
    H_strat = T_H[0] + np.einsum('spaSPA,SPA->spa', T_H[1], sigma)            \
...
def J_qre(y, u, phi, num_s, num_p, num_a_max, num_a_tot, T_y2beta, homotopy_helpers):
    
    J_mask = homotopy_helpers['J_mask']
    T_H = homotopy_helpers['T_H_qre']
    T_J = homotopy_helpers['T_J_qre']
    einsum_eqs = homotopy_helpers['einsum_eqs_qre']
