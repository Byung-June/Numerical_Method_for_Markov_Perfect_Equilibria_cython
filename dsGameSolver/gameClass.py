...
class dsGame:
    def __init__(self, payoffMatrices, transitionMatrices=None,
                 discountFactors=0, detectSymmetry=True, symmetryPairs=[],
                 checkInput=True, confirmationPrintout=True):
        # no transitionMatrix supplied: repeated game, one state
        # no discountFactors supplied:
        # one-shot game, arbitrary number of states

        if checkInput:
            self.inputCorrect = gameClass_checkInput.inputCorrect(
                payoffMatrices=payoffMatrices,
        ...
        self.nums_actions = np.zeros(
            (self.num_states, self.num_players), dtype=np.int32)
